...
export const Dashboard: React.FC<DashboardProps> = ({ user, meetings, t, lang, onNavigate, onBookForTeam, onQuickBook, onRefresh }) => {
  ...
  const startsInLabel = timeUntil
    ? `${t('startsIn')} ${timeUntil.hours > 0 ? `${timeUntil.hours}h ` : ''}${timeUntil.minutes}m`
    : '';
  ...
  return (
    <div {...pullHandlers} className="space-y-6 md:space-y-12 relative min-h-[80vh]">
      ...
      {isRefreshing && (
        <div className="absolute -top-10 left-1/2 -translate-x-1/2 z-50 flex items-center justify-center gap-2 p-3 bg-white dark:bg-gray-800 rounded-full shadow-lg text-al-adaam text-xs font-semibold animate-fade-in-down">
          ...
          <div className="hidden lg:grid grid-cols-3 gap-3 w-auto">
               {onQuickBook && (
                 ...
                 >
                    ...
                    <span className="text-sm font-semibold text-charcoal dark:text-white group-hover:text-al-adaam transition-colors">{t('quickBook')}</span>
                    <span className="text-[10px] text-gray-500">{t('nextAvailable')}</span>
                 </button>
               )}
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar scroll-smooth-mobile">
               {onQuickBook && (
                 <button
                   onClick={onQuickBook}
                   data-tour="quick-book"
                   className="group flex-shrink-0 flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-al-adaam to-al-adaam-dark text-white rounded-xl shadow-sm active:scale-95 transition-transform"
                 >
                    <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center">
                       <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                    </div>
                    <div className="text-left">
                      <span className="text-sm font-semibold block">{t('quickBook')}</span>
                      <span className="text-[10px] text-white/70">{t('nextAvailable')}</span>
                    </div>
                 </button>
               )}
               ...
               {user.role !== 'guest' && (
                 ...
                 >
                    ...
                    <div className="text-left">
                      <span className="text-sm font-semibold text-charcoal dark:text-white block">{t('team')}</span>
                      <span className="text-[10px] text-gray-500">{t('viewPeers')}</span>
                    </div>
                 </button>
               )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8 animate-slide-up delay-100">
        ...
        <div className="flex gap-3 md:gap-6 md:flex-col overflow-x-auto md:overflow-visible no-scrollbar -mx-4 px-4 md:mx-0 md:px-0">
           <div onClick={() => onNavigate('my-meetings')} className="flex-shrink-0 w-[160px] md:w-auto bg-white dark:bg-gray-800 p-4 md:p-6 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow cursor-pointer group active:scale-[0.98]">
              <div className="flex justify-between items-start mb-2 md:mb-4">
                 <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-dune">{t('pending')}</span>
              ...
              <div className="flex items-center gap-1 text-[10px] md:text-xs text-salmon font-medium">
                 ...
                 <span>{t('awaitingAction')}</span>
              </div>
           </div>

           <div onClick={() => onNavigate('my-meetings')} className="flex-shrink-0 w-[160px] md:w-auto bg-white dark:bg-gray-800 p-4 md:p-6 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow cursor-pointer group active:scale-[0.98]">
              <div className="flex justify-between items-start mb-2 md:mb-4">
                 <span className="text-[10px] md:text-xs font-bold uppercase tracking-widest text-dune">{t('approved')}</span>
           ...
           <div onClick={() => onNavigate('my-meetings')} className="md:hidden flex-shrink-0 w-[160px] bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm cursor-pointer group active:scale-[0.98]">
              <div className="flex justify-between items-start mb-2">
                 <span className="text-[10px] font-bold uppercase tracking-widest text-dune">{t('total')}</span>
        ...
        <div className="lg:col-span-2">
           <div
              className={`h-full min-h-[280px] md:min-h-[320px] rounded-2xl p-5 md:p-8 relative overflow-hidden flex flex-col justify-between group shadow-lg transition-all duration-500 ${isHappeningNow ? 'bg-al-adaam text-white' : 'bg-charcoal dark:bg-gray-800 text-white'}`}
              ...
              <div className="relative z-10 flex justify-between items-start">
                 <div className="flex items-center gap-2 md:gap-3">
                    ...
                    <span className="font-mono text-[10px] md:text-xs font-bold uppercase tracking-widest text-white/80">
                       {isHappeningNow ? t('happeningNow') : nextMeeting ? t('nextUp') : t('scheduleClear')}
                    </span>
                 </div>
                 {nextMeeting && (
                    ...
                    <>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between border-t border-white/10 pt-4 md:pt-6 gap-4">
                         <div className="flex items-center gap-3 md:gap-4">
                           <div className="w-8 h-8 md:w-10 md:h-10 rounded-full border border-white/20 bg-white/15 flex items-center justify-center text-[10px] md:text-xs font-semibold tracking-wide" aria-hidden="true">
                             {initialsOf(nextMeeting.attendeeName)}
                           </div>
                           <div>
                             <p className="text-white/50 text-[9px] md:text-[10px] font-mono uppercase tracking-widest mb-0.5">{t('attendee')}</p>
      ...
      <div className="border-t border-gray-200 dark:border-gray-700 pt-6 md:pt-8 animate-slide-up delay-200">
         <div className="flex items-center justify-between mb-4 md:mb-6">
            <h3 className="text-base md:text-lg font-display font-bold text-charcoal dark:text-white">{t('recentActivity')}</h3>
              ...
              <div key={m.id} className="flex items-center justify-between p-3 md:p-4 border-b border-gray-50 dark:border-gray-700/50 last:border-0 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors active:bg-gray-100 dark:active:bg-gray-700 press-effect">
                 <div className="flex items-center gap-3 md:gap-4 min-w-0 flex-1">
                    ...
                    <div className="min-w-0 flex-1">
                       <p className="text-sm font-semibold text-charcoal dark:text-white truncate">{m.title}</p>
                       <p className="text-xs text-gray-500 truncate">{t('with')} {m.attendeeName}</p>
                    </div>
                 </div>
                 <div className="text-right flex-shrink-0 ml-2">
                    <p className="text-[10px] md:text-xs font-mono text-charcoal dark:text-gray-300">{m.date}</p>
                    <p className={`text-[9px] md:text-[10px] uppercase font-medium ${m.status === 'approved' ? 'text-palm' : m.status === 'pending' ? 'text-salmon' : 'text-gray-400'}`}>{t(m.status) || m.status}</p>
                 </div>
              </div>
            ))}
            {meetings.length === 0 && (
              <div className="p-6 md:p-8 text-center">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500">{t('noRecentActivity')}</p>
              </div>
            )}
         </div>
      </div>
